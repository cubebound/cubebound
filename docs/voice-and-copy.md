...
**Four checks constrain user-facing copy, which is easy to forget when the
...
`check:cube-ownership` requires two of the three delete confirmations to keep
*comparing* the typed name and `check:account-deletion` the third, the one on
`/settings`; the prose around all three is free, the comparison is not.
`check:public-cube` asserts a cube page still offers Share and Clone by
those exact labels. Run all four after a copy pass.
